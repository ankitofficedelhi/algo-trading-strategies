...
cwd = Path.cwd()
nifty_future = Path("C:\\PICKLE\\SX Future")
nifty_options = Path("C:\\PICKLE\\SX Options")
...
def get_straddle_strike(
    ...
    for current_dt in valid_times:
        ...
        try:
            ...
            ce_price, pe_price = (
                opt.loc[
                    (opt["date_time"] == current_dt) & (opt["scrip"] == ce_scrip), "close"
                ].iloc[0],
                opt.loc[
                    (opt["date_time"] == current_dt) & (opt["scrip"] == pe_scrip), "close"
                ].iloc[0],
            )
...
for future_path, option_path in zip(nifty_future_list, nifty_options_list):
    ...
    os.makedirs(cwd / "B120G SX output", exist_ok=True)
    output_path = cwd / "B120G SX output"
    pd.DataFrame(combinations).to_csv(output_path / f"SX {fut.index.date[0]} B120G.csv", index=False)

    combination.extend(combinations)
pd.DataFrame(combination).to_csv(cwd / "B120G SX output" / "SX All Days B120G.csv", index=False)
