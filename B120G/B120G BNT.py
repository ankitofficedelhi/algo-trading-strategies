...
cwd = Path.cwd()
nifty_future = Path("C:\\PICKLE\\BN Future")
nifty_options = Path("C:\\PICKLE\\BN Options")
...
print(nifty_future_list[0], nifty_options_list[0])
...
def ut_check(data, sl_time, ut_sl):
    ...
    # Check if data is empty after slicing
    if data.empty:
    ...
    # Check if data is empty after removing first row
    if data.empty:
...
def b120(
    ...
    if ce_sl_time and pe_sl_time:
        if ce_sl_time < pe_sl_time:
            ...
            # Handle None returns from ut_check
            if close_price is None:
                return meta_data

            ut_pnl = pe_ut_price - close_price
            pe_pnl = pe_price - pe_sl_price - (pe_price * 0.0125)
            ce_pnl = ce_price - ce_sl_price - (ce_price * 0.0125)
            ...
            meta_data[f"UT.PL.AT.SL_{idx}"] = (
                pe_price - pe_data.loc[ce_sl_time, "close"] - (pe_price * 0.0125)
            )
            meta_data[f"Total.PNL_{idx}"] = (
                meta_data[f"TT.PL.AT.SL_{idx}"]
                + meta_data[f"UT.PL.AT.SL_{idx}"]
                + ut_pnl
            )
            ...
            meta_data[f"UT.PL.AT.SL_{idx}"] = (
                ce_price
                - ce_data.loc[pe_sl_time:, "close"].iloc[0]
                - (ce_price * 0.0125)
            )
            meta_data[f"Total.PNL_{idx}"] = (
                meta_data[f"TT.PL.AT.SL_{idx}"]
        ...
        ce_pnl = ce_price - ce_data.iloc[-1]["close"] - (ce_price * 0.0125)
        meta_data[f"CE.PNL_{idx}"] = round(ce_pnl, 2)
        meta_data[f"BPL_{idx}"] = ce_pnl + (
            pe_price - pe_data.iloc[-1]["close"] - (pe_price * 0.0125)
        )
        ...
        ut_pnl = pe_ut_price - close_price
        pe_pnl = pe_price - pe_data.iloc[-1]["close"] - (pe_price * 0.0125)
        ce_pnl = ce_price - ce_sl_price - (ce_price * 0.0125)
        meta_data[f"CE.PNL_{idx}"] = round(ce_pnl, 2)
        meta_data[f"PE.PNL_{idx}"] = round(pe_pnl, 2)
        meta_data[f"UT.PNL_{idx}"] = round(ut_pnl, 2)
        ...
        meta_data[f"TT.PL.AT.SL_{idx}"] = round(ce_pnl, 2)
        meta_data[f"UT.PL.AT.SL_{idx}"] = (
            pe_price - pe_data.loc[ce_sl_time:, "close"].iloc[0] - (pe_price * 0.0125)
        )
        meta_data[f"Total.PNL_{idx}"] = (
        ...
        meta_data[f"UT.PL.AT.SL_{idx}"] = (
            ce_price - ce_data.loc[pe_sl_time:, "close"].iloc[0] - (ce_price * 0.0125)
        )
        meta_data[f"Total.PNL_{idx}"] = round(
            (pe_pnl + meta_data[f"UT.PL.AT.SL_{idx}"] + ut_pnl), 2
        )
...
def b120_intraday(
    ...
    if om != 0:
        ce_scrip, pe_scrip, ce_price, pe_price, future_price, current_dt = (
            get_strangle_strike(option, future, start_time, end_time, 100, om)
        )
    else:
        ce_scrip, pe_scrip, ce_price, pe_price, future_price, current_dt = (
            get_straddle_strike(option, future, start_time, end_time, 100)
        )
    ...
    # print(option.head(2))
    option = option.loc[current_dt:end_time]

    option = option = option[
        option.index > current_dt
    ]  # to avoid entry time data in sl hit

    ce_data = option[option["scrip"] == ce_scrip]
    pe_data = option[option["scrip"] == pe_scrip]

    print("CE", ce_scrip, "PE", pe_scrip, current_dt, om, sl, ut_sl)
...
end_times_1 = pd.to_datetime(
    param_df.end_time_1.dropna().unique(), format="%H:%M:%S"
).time
...
end_times_2 = pd.to_datetime(
    param_df.end_time_2.dropna().unique(), format="%H:%M:%S"
).time
...
for future_path, option_path in zip(nifty_future_list, nifty_options_list):
    ...
    # Ensure monotonic index to allow safe range operations
            ...
            for sl_1, sl_2 in zip(sls_1, sls_2):
                for ut_sl_1, ut_sl_2 in zip(ut_sls_1, ut_sls_2):
                    for om_1, om_2 in zip(oms_1, oms_2):
                        ...
                        data_1 = b120_intraday(
                            option, fut, start_dt_1, end_dt_1, sl_1, ut_sl_1, om_1, 1
                        )
                        data_2 = b120_intraday(
                            option, fut, start_dt_2, end_dt_2, sl_2, ut_sl_2, om_2, 2
                        )

                        if data_1 and data_2:
                            data_1.update(data_2)
                            data_1["Total.PNL"] = round(
                                (data_1["Total.PNL_1"] + data_1["Total.PNL_2"]), 2
                            )
                            ...
                            print(
                                f"Skipping combination: both periods returned None for date {file_date}"
                            )

    df = pd.DataFrame(combinations)
    os.makedirs(cwd / "B120G BANK NIFTY output", exist_ok=True)
    output_path = cwd / "B120G BANK NIFTY output"
    pd.DataFrame(combinations).to_csv(
        output_path / f"BANK NIFTY {fut.index.date[0]} B120G.csv", index=False
    )

    combination.extend(combinations)
pd.DataFrame(combination).to_csv(
    cwd / "B120G BANK NIFTY output" / "BANK NIFTY All Days B120G.csv", index=False
)
